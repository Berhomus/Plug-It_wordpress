					...
					try
					{
						...
						while($ar=$rq->fetch())
						{
							...
							if($i == 1)
								...
								<span style="margin-right:18px;float:right;">HT : '.(round($ar['prix']*100)/100).'€ | TVA ('.round($tva['valeur']*100)/100 .'%) : '.(round($ar['prix']*($tva['valeur']/100)*100)/100).' € | TTC : '.(round($ar['prix']*(($tva['valeur']/100)+1)*100)/100).' €</span>
								</p>
								</div>
